...
void render(Game *game)
{
	...
	glColor3ub(221,72,20); //Ubuntu Orange
	...
	for(int i=0; i<5; i++) {
		...
		glPopMatrix();
            ...
            if(Dtrue) {
		r[5].bot = WINDOW_HEIGHT-500;
		r[5].left = 10;
		r[5].center = 0;
		ggprint16(&r[5], 120, 0xffffff, "'d' has been pressed.. double time!");
}
	for(int j=0; j < 5; j++) {
		s=&game->box[j];
		r[j].bot = s->center.y-10;
		r[j].left = s->center.x;
		r[j].center = 1;
		int cref = 0xFFFFFF;
		if(j==0){
			ggprint16(&r[j], 16, cref, "Requirments");
		}

		if(j==1) {
			ggprint16(&r[j],16,cref,"Design");
		}
		if(j==2) {
			ggprint16(&r[j],16,cref,"Coding");
		}
		if(j==3) {
			ggprint16(&r[j],16,cref,"Testing");
		}
		if(j==4) {
			ggprint16(&r[j],16,cref,"Maintenance");
		}
	}
